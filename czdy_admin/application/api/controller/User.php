...
class User extends Api
{
    ...
    public function _initialize()
    {
        ...
        // 临时：自动添加字段
        try {
            $columns = \think\Db::query("SHOW COLUMNS FROM fa_user LIKE 'gender'");
            if (empty($columns)) {
                \think\Db::execute("ALTER TABLE fa_user ADD COLUMN gender ENUM('0','1','2') DEFAULT '0' COMMENT '性别' AFTER nickname");
            }
            $columns = \think\Db::query("SHOW COLUMNS FROM fa_user LIKE 'birthday'");
            if (empty($columns)) {
    ...
    public function register()
    {
        ...
        // 图形验证码校验（使用与 /api/common/captcha 相同的配置和事件名 register）
        if (!$captcha) {
            $this->error(__('Captcha is incorrect'));
        }
        // 这里不要使用全局 captcha_check 助手（它走的是前台 index 模块的验证码）
        // 而是与 app\api\controller\Common::captcha 保持一致
        $captchaObj = new Captcha((array)Config::get('captcha'));
        if (!$captchaObj->check($captcha, 'register')) {
            $this->error(__('Captcha is incorrect'));
        }
    ...
    public function profile()
    {
        ...
        $bio = $this->request->post('bio', null);
     ...
     *
     * @ApiMethod (POST)
     * @ApiParams (name="oldpassword", type="string", required=true, description="当前密码")
     * @ApiParams (name="newpassword", type="string", required=true, description="新密码")
     */
    public function changepwd()
    {
        ...
        if ($ret) {
            $this->success(__('Password changed successful'));
        } else {
            $this->error($this->auth->getError() ?: __('Password change failed'));
        }
    }
}
